...
describe('ResponseBodyViewer.vue', () => {
    ...
    it('renders body and toggles format mode', async () => {
        ...
        // Switch to Raw
        const rawBtn = wrapper.findAll('.action-btn').find(b => b.text().trim() === 'Raw');
        await rawBtn?.trigger('click');
        await wrapper.vm.$nextTick();
        expect(wrapper.find('.mock-monaco').text()).toContain('{"foo":"bar"}');

        // Switch back to Pretty
        const prettyBtn = wrapper.findAll('.action-btn').find(b => b.text().trim() === 'Pretty');
        await prettyBtn?.trigger('click');
        await wrapper.vm.$nextTick();
        expect(wrapper.find('.mock-monaco').text()).toContain('"foo": "bar"');
    });
    ...
    it('renders headers tab', async () => {
        ...
        // Switch back to Body
        const bodyBtn = wrapper.findAll('.tab').find(b => b.text().includes('Body'));
        await bodyBtn?.trigger('click');
        await wrapper.vm.$nextTick();
        expect(wrapper.find('.mock-monaco').exists()).toBe(true);
    });
